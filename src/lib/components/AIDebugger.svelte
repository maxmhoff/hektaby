<script lang="ts">
	import { aiSolution } from "$lib/stores/gameStore";
    let isActive = false;
...
{#if isActive}
    <div class="ai-debugger">
        ...
        <button class="ai-debugger__close" on:click={() => (isActive = false)}>&#x2715</button>
        ...
        <p class="ai-debugger__item">{`tileOrder: ${$aiSolution.tileOrder}`}</p>
